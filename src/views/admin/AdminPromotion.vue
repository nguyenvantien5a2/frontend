        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="promo in promotions" :key="promo.id" class="hover:bg-gray-50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap">
              <img :src="promo.image_url || 'https://via.placeholder.com/100?text=No+Img'" class="h-12 w-20 object-cover rounded-md border border-gray-200" alt="Promo" />
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center">
              <span v-if="promo.discount_value" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-800 border border-red-200">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
              <div class="flex flex-col space-y-1">
                <span class="text-xs font-medium text-gray-500">BĐ: <span class="text-gray-700">{{ formatDate(promo.start_date) }}</span></span>
                <span class="text-xs font-medium text-gray-500">KT: <span class="text-gray-700">{{ formatDate(promo.end_date) }}</span></span>
              </div>
            </td>
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto animate-fade-in-up">
        ...
        <form @submit.prevent="savePromotion" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="form.image_url" type="text" class="mt-1 block w-full border rounded-lg px-3 py-2" placeholder="https://..." />
            <div v-if="form.image_url" class="mt-2">
              <img :src="form.image_url" class="h-20 w-full object-cover rounded border" onerror="this.style.display='none'" />
            </div>
          </div>
...
const openModal = (promo = null) => {
  if (promo) {
    ...
    form.value = { 
      ...
      discountPercent: null, startDate: '', endDate: '', image_url: '' 
    }
  }
