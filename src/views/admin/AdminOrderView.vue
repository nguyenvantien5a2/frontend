          ...
          <tr v-for="order in paginatedOrders" :key="order.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <div class="flex justify-end items-center space-x-3">
                ...
                <!-- Icon Xóa (Thùng rác) - Chỉ hiện khi PENDING hoặc CANCELLED -->
                <button 
                ...
                <!-- Placeholder để giữ khoảng cách nếu không có nút xóa -->
        ...
        <div v-if="selectedOrder" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg">
            <div>
              ...
              <p class="font-medium text-gray-900">{{ selectedOrder.customerName }}</p>
            </div>
            <div>
            ...
            <div class="md:col-span-2">
              ...
              <p class="font-medium text-gray-900">{{ selectedOrder.shippingAddress }}</p>
            </div>
            <div>
              <p class="text-xs text-gray-500 uppercase font-semibold mb-1">Thanh toán</p>
              <p class="font-bold" :class="getPaymentMethodClass(selectedOrder.paymentMethod)">
                  ...
                  <tr v-for="item in selectedOrder.orderDetails" :key="item.id">
                    <td class="px-4 py-3">
                      <div class="flex items-center">
                        <img :src="item.product.imageUrl || 'https://via.placeholder.com/40'" class="w-10 h-10 object-cover rounded border mr-3">
                        <div>
...
const updateStatus = async (order, event) => {
  ...
  try {
    ...
    order.status = newStatus // Cập nhật UI nếu thành công
    toastStore.success(`Đã cập nhật trạng thái đơn hàng #${order.id}`)
  } catch (error) {
    console.error("Lỗi cập nhật trạng thái:", error)
    event.target.value = oldStatus // Hoàn tác nếu lỗi
...
const getPaymentMethodText = (method) => {
  if (method && (method.toUpperCase() === 'MOMO' || method === 'MoMo')) {
    return 'Ví MoMo'
  }
