        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="product in products" :key="product.id" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <img :src="getImageUrl(product.imageUrl)" class="h-12 w-12 rounded-lg object-cover border flex-shrink-0" alt="">
            </td>
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
        ...
        <form @submit.prevent="saveProduct" class="space-y-4">
          ...
          <div>
            ...
            <!-- Preview ảnh -->
            <div v-if="previewImage || form.imageUrl" class="mt-2">
              <img :src="previewImage || getImageUrl(form.imageUrl)" class="h-20 w-20 object-cover rounded border" />
            </div>
          </div>
...
const form = ref({
  id: null, name: '', price: 0, stock: 0, imageUrl: '', description: '', categoryId: null
})
...
const fetchProducts = async (page = 0) => {
  loading.value = true
  try {
    // Gọi API admin để lấy danh sách (hoặc dùng API public nếu chưa phân quyền chặt)
    const res = await api.get('/admin/products', {
      params: { page, size: pageSize.value, keyword: searchKeyword.value, categoryId: selectedCategoryId.value }
    })
    // Cập nhật state từ response Page<Product>
    ...
    toastStore.error('Không thể tải danh sách sản phẩm. Vui lòng kiểm tra lại server hoặc quyền truy cập.')
  } finally {
    loading.value = false
  }
}
...
const handleSearch = () => {
  fetchProducts(0) // Tìm kiếm thì luôn quay về trang 1
}
...
const deleteProduct = async (id) => {
  ...
  try {
    ...
    if (error.response && error.response.data) {
      toastStore.error(typeof error.response.data === 'string' ? error.response.data : 'Xóa thất bại. Sản phẩm có thể đang được sử dụng.')
    } else {
      toastStore.error('Xóa thất bại. Vui lòng thử lại.')
    }
  }
}
...
const openModal = (product = null) => {
  ...
  if (product) {
    ...
    form.value = { ...product, categoryId: product.category?.id || null } // Map category object to ID
  } else {
    isEditing.value = false
    form.value = { name: '', price: 0, stock: 10, imageUrl: '', description: '', categoryId: null }
  }
...
const handleFileUpload = (event) => {
  ...
  if (file) {
    // Validate file (giống ProfileView)
...
const saveProduct = async () => {
  ...
  try {
    ...
    // Chuẩn bị object product để gửi đi
    // Tạo payload mới sạch sẽ để tránh gửi các trường thừa (như createdAt, updatedAt, circular refs)
    ...
    // Gửi object product dưới dạng JSON string
    ...
    // Cấu hình header để browser tự động set boundary cho multipart/form-data
    // Việc set Content-Type: undefined sẽ ghi đè default application/json của axios (nếu có)
    const config = {
      headers: {
    ...
    if (isEditing.value) {
      await api.put(`/admin/products/${form.value.id}`, formData, config)
      // Update list (load lại để đảm bảo dữ liệu mới nhất)
      fetchProducts(currentPage.value)
      toastStore.success('Cập nhật thành công!')
    } else {
      await api.post('/admin/products', formData, config)
      fetchProducts(0) // Về trang đầu sau khi thêm mới
    ...
    if (error.response && error.response.data) {
        toastStore.error(typeof error.response.data === 'string' ? error.response.data : 'Có lỗi xảy ra khi lưu sản phẩm.')
    } else {
        toastStore.error('Có lỗi xảy ra. Vui lòng kiểm tra lại kết nối.')
    }
  }
}
